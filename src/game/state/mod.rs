...
impl GameState {
    ...
    pub fn check_input(&mut self, render_state: &RenderState) {
        ...
        self.player_rotation[0] += (rot_vec[0] * 0.02) + render_state.input.xrel();
        self.player_rotation[1] += (rot_vec[1] * 0.02) + render_state.input.yrel();
        self.player_rotation[2] += rot_vec[2] * 0.02;

        let view = glam::Mat4::from_rotation_y(-self.player_rotation[0]);
        let view = view * glam::Mat4::from_rotation_x(-self.player_rotation[1]);
